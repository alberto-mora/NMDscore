...
clin = pd.read_csv('/home/vant/Braun/Clinical_and_Immune_Data.csv',sep='\t') #Datos clínicos recogidos de Braun et al., 2020, documento llamado Table S1. Clinical and immune phenotype data for the CheckMate cohorts.
df = pd.read_csv('/home/vant/Braun/NMDB_Braun.csv',sep='\t') #dataframe con variantes somáticas con el NMD score estraido gracias al script Braun_ExtractNMDscorefromGTF_PTC
rna = pd.read_csv('/home/vant/Braun/RNA_Braun.csv', sep='\t') #Datos de expresión recogidos de Braun et al., 2020 sacado del documento llamado Table S4. RNA expression (normalized expression matrix).
cs = pd.read_csv('/home/vant/Braun/Braun_CIBERSORTX.csv', sep='\t') #Datos de deconvolution inmune recogidos de Braun et al., 2020 sacado del documento llamado Table S4. RNA expression (normalized expression matrix).
...
###Si se decide analizar datos para fsINDELs sin clasificar no hacer este paso
df = df.loc[df['NMDB'] != '.']
...
U1, p = mannwhitneyu(plotdf['count'].loc[plotdf['InmunoPheno'].isin(['Infiltrated'])], plotdf['count'].loc[plotdf['InmunoPheno'].isin(['Desert'])], alternative = 'greater')
###fsINDELs tras clasificarlos
...
desert_t= plotdf_aux['triggersNMD'].loc[plotdf_aux['InmunoPheno'] == 'Desert']
excluded_t =  plotdf_aux['triggersNMD'].loc[plotdf_aux['InmunoPheno'] == 'Excluded']
infiltrated_t =  plotdf_aux['triggersNMD'].loc[plotdf_aux['InmunoPheno'] == 'Infiltrated']

#p value en distinta categoría
U1, p = mannwhitneyu(plotdf2_aux['notriggersNMD'].loc[plotdf2_aux['InmunoPheno'] == 'Infiltrated'], plotdf2_aux['notriggersNMD'].loc[plotdf2_aux['InmunoPheno'] == 'Desert'], alternative = 'greater')
U3, p3 = mannwhitneyu(plotdf_aux['triggersNMD'].loc[plotdf_aux['InmunoPheno'] == 'Infiltrated'], plotdf_aux['triggersNMD'].loc[plotdf_aux['InmunoPheno'] == 'Desert'], alternative = 'greater')
...
pos = [1,2,4,5]
fig, ax = plt.subplots()
parts = ax.violinplot([desert, desert_t, infiltrated, infiltrated_t], pos, showmedians = True)
