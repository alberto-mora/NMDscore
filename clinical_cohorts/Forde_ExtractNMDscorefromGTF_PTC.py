...
##Abre el archivo csv
filename = '/home/vant/Forde/Forde_2.csv' #Dataframe con las variantes somáticas, se puede descargar de Forde et al., 2018
...
df = df.loc[df['Consequence'].str.contains('Nonsense|Frameshift')]


df['VEP_allele'] = df[REF] + '/' + df[ALT]
df['ID_specific'] = df[chr].astype(str) + '_' + df[position].astype(str) + '_' + df['VEP_allele'] + '_' + df['Sample ID']
...
df['PTCposition'] = df.apply(lambda x: getPTCpositionFromFS(gtf, x['ID'], refgen, x['Mutation Type'], x['ID_specific']), axis=1)
...
df['NMDB'] = df.apply(lambda x: nmdxtract(x['ID'], x['PTCposition'], gtf, x['Consequence']), axis=1)
df['NMDA'] = df.apply(lambda x: nmdxtract(x['ID'], x['PTCposition'], gtfA, x['Consequence']), axis=1)


# Save NMD-annotated df (NMD_Braun.csv)

df.to_csv('new_NMD_Forde.csv',sep='\t',index = None)
