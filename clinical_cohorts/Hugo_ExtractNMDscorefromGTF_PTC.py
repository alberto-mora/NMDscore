...
##Abre el archivo csv
filename = '/home/vant/Hugo/Hugo_2.csv' #Dataframe con las variantes somáticas, se puede descargar de Hugo et al., 2016.

## Carga archivos de referencia  Archivo GTF se puede descargar de https://figshare.com/articles/dataset/NMDetective/7803398
...
df[['Tumor_Seq_Allele1','Tumor_Seq_Allele2']] = df['NucMut'].str.split('>',1,expand=True)
...
df = df.loc[df['MutType'].str.contains('Nonsense|Frame_Shift')]


df['VEP_allele'] = df[REF] + '/' + df[ALT]
df['ID_specific'] = df[chr].astype(str) + '_' + df[position].astype(str) + '_' + df['VEP_allele'] + '_' + df['Sample']
...
df['PTCposition'] = df.apply(lambda x: getPTCpositionFromFS(gtf, x['ID'], refgen, x['MutType'], x['ID_specific']), axis=1)

df = df.reset_index()
df.drop('index',1, inplace = True)


df['NMDB'] = df.apply(lambda x: nmdxtract(x['ID'], x['PTCposition'], gtf, x['MutType']), axis=1)
df['NMDA'] = df.apply(lambda x: nmdxtract(x['ID'], x['PTCposition'], gtfA, x['MutType']), axis=1)


df.to_csv('new_NMD_Hugo.csv',sep='\t',index = None)
