...
##Abre el archivo csv
filename = '/home/vant/Van_allen/Van_allen_2.csv' #Dataframe con las variantes somáticas, se puede descargar de Van allen et al., 2015
...
def nmdxtract(ID, PTCposition, mega_gtf, consequence):
	...
	if 'Nonsense' in consequence:
		...
		if (PTCposition != '.') & (PTCposition != 'utr') & (PTCposition != 'out') & (PTCposition != 'raro'):
			POS = int(PTCposition)
			mega_gtf = mega_gtf.loc[(mega_gtf['seqname'] == CHR) & (mega_gtf['start'] <= POS) & (mega_gtf['end'] >= POS)]
			if mega_gtf.empty == False:
	...
	if 'Nonsense' in consequence:
		...
		if gtf.loc[(gtf['seqname'] == CHR) & (gtf['start'] <= POS) & (gtf['end'] >= POS)].empty == False:
			...
			if 'Del' in consequence:
				...
				if dele in wild_df.index.values:
...
df = df.loc[df['Chromosome'] != 'GL000218.1']


df['VEP_allele'] = df[REF] + '/' + df[ALT]
df['ID_specific'] = df[chr].astype(str) + '_' + df[position].astype(str) + '_' + df['VEP_allele'] + '_' + df['patient']
...
df.to_csv('new_NMDB_Van_Allen.csv',sep='\t',index = None)
